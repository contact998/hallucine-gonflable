...
export const viseeNeuve = (cible = -1.0): Visee => ({ cible, anime: false });
 ...
 */
export function azimutPourCote(m: Modele, cote: string): number {
  return (90 - angleCote(m, cote)) * RAD;
}

/** Viser ce côté, à partir de maintenant. */
export function viser(v: Visee, m: Modele, cote: string): void {
 ...
 */
export function prochainAzimut(v: Visee, courant: number): number | null {
  if (!v.anime) return null;
