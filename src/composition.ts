 ...
 */
export function cleImpressionCote(m: Modele, taille: string, imp: Impression, cote: string): string | null {
  if (!m.cleParCote) return null;
  if (IMPRESSIONS[imp].startsWith("impression-paroi-courbe")) return null;
 ...
 */
export function impressionDuCote(
  m: Modele,
  taille: string,
  type: string,
  cote: string,
  connait: (cle: string) => boolean,
): { imp: Impression; cle: string } | null {
  for (const imp of impressionsCote(type)) {
    const lettree = cleImpressionCote(m, taille, imp, cote);
    if (lettree && connait(lettree)) return { imp, cle: lettree };
    const generique = cleImpression(m, taille, imp);
    if (connait(generique)) return { imp, cle: generique };
  }
  return null;
}
 ...
 */
export function cleImpressionDemiMur(
  m: Modele,
  taille: string,
  connait: (cle: string) => boolean,
): string | null {
  const d = m.demiMur;
  if (!d) return null;
  const generique = cleImpression(m, taille, d.impression as Impression);
  const lettree = `${generique}-${d.lettre}`;
  if (connait(lettree)) return lettree;
