 ...
 */
function unitesTriees(panier: LigneLounge[], catalogue: Record<string, MobilierItem>): Unite[] {
  const unites: Unite[] = [];
  for (const ligne of panier) {
    const item = catalogue[ligne.slug];
    if (!item) continue;
    for (let i = 0; i < ligne.qte; i++) unites.push({ slug: ligne.slug, index: i, item });
  }
  unites.sort((a, b) => (a.slug === b.slug ? a.index - b.index : a.slug < b.slug ? -1 : 1));
  return unites;
}

function dimsDepuisSurface(surfaceM2: number): { largeurM: number; profondeurM: number } {
 ...
 *
 * Pourquoi cette indirection. Le rangeur posait des meubles un à un, en
 * rangées : dense, et parfaitement laid — quinze canapés alignés dans le même
 * sens ressemblent à un entrepôt, pas à un lounge. Un îlot de deux assises face
 * à face est UN rectangle pour lui, et redevient deux meubles orientés l'un
 * vers l'autre à l'arrivée. Le rangement, lui, ne change pas : c'est celui qui
 * a passé le contrôle exhaustif de non-chevauchement, et il continue de ne voir
 * que des rectangles.
 */
interface Bloc {
  wM: number;
  dM: number;
  unites: Unite[];
  /** Déplie le bloc en meubles, autour du centre (x, z) que le rangeur lui a donné. */
  deplier: (x: number, z: number) => MeublePose[];
}
...
function centrer(
  meubles: MeublePose[],
  catalogue: Record<string, MobilierItem>,
  largeurM: number,
  profondeurM: number,
): MeublePose[] {
  if (meubles.length === 0) return meubles;
  ...
  for (const m of meubles) {
    ...
    xMin = Math.min(xMin, m.x - w / 2); xMax = Math.max(xMax, m.x + w / 2);
    zMin = Math.min(zMin, m.z - d / 2); zMax = Math.max(zMax, m.z + d / 2);
  }
  if (!Number.isFinite(xMin)) return meubles;
  ...
  const marge = (v: number, demi: number, min: number, max: number) =>
    max - min > 2 * demi ? 0 : v;
...
function blocSimple(u: Unite): Bloc {
  return {
...
function blocTablee(table: Unite, chaises: Unite[], _passage: number): Bloc {
  ...
  return {
    ...
    deplier: (x, z) => {
      ...
      const ecart = (d: number) => d / 2 + JEU_TABLE_M + dC / 2;
...
interface ResultatPack {
  placements: MeublePose[];
  echecs: Unite[];
  finalRowZ: number;
  finalRowMaxDepth: number;
}
 ...
 */
function largeurZoneEstimeeBlocs(blocs: Bloc[], circulation: number): number {
  if (blocs.length === 0) return circulation;
...
export function implanter(
  ...
  if (emprise && emprise.largeurM > 0 && emprise.profondeurM > 0) {
    ...
    const mesurePrincipale = packZonePrincipale(
      { xMin: 0, xMax: largeurPrincipale, zMin: 0, zMax: Infinity },
      blocsAssises,
      blocsDebout,
      CIRCULATION_M,
      enRangs,
  ...
  if (bars.length > 0) {
    const rectBars: Rect = { xMin: xHalf - largeurColonneBarEff, xMax: xHalf, zMin: -zHalf, zMax: zHalf };
  ...
  const brut: MeublePose[] = [...resultatPrincipal.placements, ...placementsBars];
 ...
 */
export function peutAccueillir(
  panier: { slug: string; qte: number }[],
  catalogue: Record<string, MobilierItem>,
  slug: string,
  surfaceM2?: number,
  emprise?: { largeurM: number; profondeurM: number },
 ...
 */
export const ANCRAGES: Record<string, { semelleY: number; fessesY?: number; talonZ?: number; dosZ?: number }> = {
 ...
 */
export const LACET_MEUBLE: Record<string, number> = {
 ...
 */
function elevationAssis(m: ModeleSilhouette, assiseM: number): number {
...
export function personnes(
  ...
  const accepter = (p: Personne): void => {
    for (const q of out) {
      if (p.duMeuble >= 0 && q.duMeuble === p.duMeuble) continue;
      const dx = p.x - q.x, dz = p.z - q.z;
      if (dx * dx + dz * dz < (2 * RAYON) ** 2) return;
    }
    for (let j = 0; j < meubles.length; j++) {
      ...
      if (u < it.largeurCm / 200 + RAYON && v < it.profondeurCm / 200 + RAYON) return;
    }
    out.push(p);
  };
 ...
 */
export function modeleSilhouette(assis: boolean, index: number): ModeleSilhouette {
