...
import { archeModelisee, type GammeArche3D } from "./arche.js";
...
import { composerFaceArche, type CotesArche } from "./archePlan.js";
...
type Outils = {
  arche: ArcheCharge;
  flanc: THREE.MeshStandardMaterial;
  avant: THREE.MeshStandardMaterial;
  arriere: THREE.MeshStandardMaterial;
  homme: THREE.Group;
  anisotropie: number;
  cadrer: (dim: { largeurM: number; hauteurM: number; profondeurM: number }, face: "avant" | "arriere") => void;
};

export default function ArcheViewer({
  forme, largeurM, hauteurM, profondeurM, teinte = "blanc",
  visuelAvant = null, visuelArriere = null, face = "avant", silhouette = true,
  captureRef, labelChargement, labelEchec, labelReessayer, libellesOutils,
}: Props) {
  ...
  const outils = useRef<Outils | null>(null);
  ...
  useEffect(() => {
    ...
    setPret(false);
    setEchec(false);
    if (!modelisee) { setEchec(true); return; }
    ...
    let dernier: { dim: { largeurM: number; hauteurM: number; profondeurM: number }; face: "avant" | "arriere" } | null = null;
    ...
    const cadrer = (dim: { largeurM: number; hauteurM: number; profondeurM: number }, f: "avant" | "arriere") => {
      dernier = { dim, face: f };
      const a = f === "avant" ? AZIMUT_AVANT : Math.PI + AZIMUT_AVANT;
      const direction = new THREE.Vector3(
        Math.sin(POLAIRE) * Math.sin(a), -Math.sin(POLAIRE) * Math.cos(a), Math.cos(POLAIRE),
      );
      const cible = new THREE.Vector3(0, 0, dim.hauteurM * 0.47);
      const demi = dim.largeurM / 2 + 0.3;
      const boite = new THREE.Box3(
        new THREE.Vector3(-demi, -dim.profondeurM, 0),
        new THREE.Vector3(demi, dim.profondeurM, dim.hauteurM),
      );
    ...
    const redimensionner = () => {
      ...
      if (dernier) cadrer(dernier.dim, dernier.face);
    };
    redimensionner();
    ...
    const chargerArche = chargerArcheGlb(loader, forme).then((a) => {
      ...
      a.groupe.traverse((o) => {
        ...
        if (maille.isMesh) maille.material = materiaux;
      });
      sc.add(a.groupe);
      return a;
    });
    ...
    return () => {
      ...
      ro.disconnect();
      orbite.dispose();
      for (const m of [flanc, avant, arriere]) { m.map?.dispose(); m.dispose(); }
      rendu.dispose();
  ...
  useEffect(() => {
    ...
    const cotes: CotesArche = { forme, largeurCm: largeurM * 100, hauteurCm: hauteurM * 100, profondeurCm: profondeurM * 100 };
    o.flanc.color.set(hex);
    let vivant = true;

    const habiller = (mat: THREE.MeshStandardMaterial, pose: VisuelPose | null, miroir: boolean) => {
      if (!pose) {
        mat.map?.dispose();
        mat.map = null;
        mat.color.set(hex);
  ...
  return (
    ...
    <div ref={hote} className="relative w-full h-full" style={{ backgroundColor: FOND_SCENE }}>
      ...
      {echec && (labelEchec || labelReessayer) && (
        <span className="absolute inset-0 flex flex-col items-center justify-center gap-3 px-4 text-center text-sm text-[#2E4A5E]/70">
          {labelEchec}
          {modelisee && labelReessayer && (
            <button
              type="button"
              onClick={() => setEssai((n) => n + 1)}
              className="rounded-lg border border-[#2E4A5E]/30 bg-white/70 px-3 py-1.5 text-[#2E4A5E] transition-colors hover:bg-[#2E4A5E]/10"
            >
              {labelReessayer}
            </button>
          )}
