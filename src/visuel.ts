...
import { plageTaille, type VisuelPose } from "./pose.js";
...
function recolorer(image: SourceImage, hex: string): HTMLCanvasElement {
...
export class ErreurVisuel extends Error {
  constructor(public readonly cause_: EchecVisuel) {
    super(cause_);
  }
}
...
function toileDe(image: SourceImage, facteur: number): HTMLCanvasElement {
  const toile = document.createElement("canvas");
 ...
 *  deux réductions. */
function pngSousPoids(toile: HTMLCanvasElement): string {
  let source = toile;
 ...
 */
export async function importerVisuelDetaille(
  fichier: File,
  { detourer = true }: OptionsImport = {},
): Promise<VisuelImporte> {
  if (!FORMATS.test(fichier.type)) throw new ErreurVisuel("format");
  if (fichier.size > POIDS_MAX) throw new ErreurVisuel("poids");
  ...
  const ctx = toile.getContext("2d");
  if (!ctx) throw new ErreurVisuel("illisible");
...
export async function importerVisuel(fichier: File, options?: OptionsImport): Promise<string> {
