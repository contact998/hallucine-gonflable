 ...
 */
function marquerFace(scene: THREE.Object3D, azimut: number, cote: string): void {
  ...
  scene.traverse((o) => {
    ...
    let plan = -Infinity;
    for (let i = 0; i < pos.count; i++) plan = Math.max(plan, p.fromBufferAttribute(pos, i).dot(dir));
    const surLaFace = (i: number) => {
    ...
    for (let i = 0; i < nb; i++) if (!surLaFace(i)) return;
...
const ZONE_PAR_PIECE: Record<string, string> = Object.fromEntries(
  [...ZONES_COULEUR, ZONE_AUVENT].map((z) => [z.piece, z.cle]),
);
 ...
 */
function projeterDuDessus(groupe: THREE.Object3D): void {
  const mailles: THREE.Mesh[] = [];
  groupe.traverse((o) => {
    const m = o as THREE.Mesh;
    if (m.isMesh && m.geometry.getAttribute("position")) mailles.push(m);
  });
  if (!mailles.length) return;
  ...
  for (const m of mailles) {
    if (m.geometry.getAttribute("uv1")) continue; // déjà posée
...
function ratioEmprise(groupe: THREE.Object3D): number {
...
function charger(loader: GLTFLoader, m: ReturnType<typeof trouverModele>, nom: string): Promise<THREE.Group> {
  ...
  if (!p) {
    p = loader.loadAsync(url).then((g) => {
      ...
      const lignes: [THREE.Mesh, LineSegments2][] = [];
      g.scene.traverse((o) => {
        const maille = o as THREE.Mesh;
      ...
      if (porteVitre(nom)) marquerVitre(g.scene);
...
export default function TenteViewer({ cotes, auvents, demiMurs, couleurs, couleursCote, visuels, visuelsCote, modele, taille, actif, labelChargement, labelEchec = "Vue 3D incomplète : un élément n’a pas pu être chargé.", labelReessayer = "Réessayer", captureRef, tentesReliees, libellesOutils, nomFichierImage }: TenteViewerProps) {
  ...
  const piecesSocle = useRef<Record<string, THREE.Group>>({});
  ...
  const piecesAffichees = useRef<{ nom: string; groupe: THREE.Group; cote?: string; voisin?: boolean }[]>([]);
  ...
  const pans = useRef<Map<string, THREE.CanvasTexture>>(new Map());
  ...
  useEffect(() => {
    ...
    const cadrer = () => {
      ...
      if (vueFixee.current) { appliquerVue(vueFixee.current, false); return; }
      ...
      cam.position.copy(centre).add(
        new THREE.Vector3(Math.cos(az) * 0.937, Math.sin(az) * 0.937, 0.35).multiplyScalar(recul),
      );
      orbite.update();
    };
    ...
    const appliquerVue = (vue: Vue, anime: boolean) => {
      const boite = new THREE.Box3().setFromObject(racine);
      if (boite.isEmpty()) return;
      const face = azimutPourCote(M, (M.cotes as readonly string[])[0]);
    ...
    const tournerCamera = (nv: number) => {
      const off = cam.position.clone().sub(orbite.target);
      const rH = Math.hypot(off.x, off.y);
      cam.position.set(
        orbite.target.x + rH * Math.cos(nv),
        orbite.target.y + rH * Math.sin(nv),
        cam.position.z,
      );
    };
    const boucle = () => {
  ...
  const imagePour = (url: string): Promise<HTMLImageElement> => {
    const dejaLa = images.current.get(url);
    if (dejaLa) return Promise.resolve(dejaLa);
    return chargerImage(url).then((img: HTMLImageElement) => {
      images.current.set(url, img);
      return img;
    });
  };

  const appliquerVisuels = useRef<() => void>(() => {});
  useEffect(() => {
    appliquerVisuels.current = () => {
      ...
      for (const { nom, groupe, cote } of piecesAffichees.current) {
        ...
        const cibles: THREE.Mesh[] = [];
        groupe.traverse((o) => {
          const maille = o as THREE.Mesh;
          if (!maille.isMesh || estVitre(maille)) return;
          if (!maille.geometry.getAttribute("uv")) return;
          const mat = maille.material as THREE.MeshStandardMaterial;
          if (!mat?.color) return;
          if (!(mat as any).__propre) {
            const copie = mat.clone();
            (copie as any).__propre = true;
        ...
        void imagePour(pose.url)
          .then((image) => {
            if (marque !== generation.current) return;
            ...
            for (const maille of cibles) {
              ...
              const donnees = geo.userData as {
                ratioGabarit?: number;
                centreTissu?: { x: number; y: number };
              };
              ...
              if (!tex) {
                tex = new THREE.CanvasTexture(
                  composerPan(image, pose, donnees.ratioGabarit, fond, cible),
                );
                tex.colorSpace = THREE.SRGBColorSpace;
  ...
  useEffect(() => {
    ...
    for (const zone of ZONES_COULEUR) {
      ...
      groupe.traverse((o) => {
        ...
        (maille.material as THREE.MeshStandardMaterial).color.copy(hex);
      });
    }
  }, [couleurs, pret]);
  ...
  useEffect(() => {
    ...
    /* `coteGeo` place la pièce, `cotePeinture` la peint et reçoit son visuel :
       ...
       image comprises. */
    const poser = (dans: THREE.Group, nom: string, coteGeo: string, cotePeinture: string, teinte: string | undefined, voisin: boolean) => {
      charger(loader, M, nom).then((g) => {
        ...
        if (teinte) {
          ...
          g.traverse((o) => {
            const maille = o as THREE.Mesh;
            const mat = maille.material as THREE.MeshStandardMaterial;
            if (!maille.isMesh || estVitre(maille) || !mat?.color) return;
            const copie = mat.clone();
            (copie as any).__propre = true;
        ...
        if (!voisin && coteGeo === actif) {
          g.traverse((o) => {
            const maille = o as THREE.Mesh;
            const mat = maille.material as THREE.MeshStandardMaterial | undefined;
            if (estVitre(maille) || !mat?.color) return;
    ...
    murs.clear();
    piecesAffichees.current = piecesAffichees.current.filter(
      (p) => !p.voisin && (VUE.socle as readonly string[]).includes(p.nom),
    );
    ...
    tentes.forEach((t, i) => {
      ...
      if (voisin && pas) {
        ...
        for (const nomSocle of VUE.socle as readonly string[]) {
      ...
      /* Le mur du bout d'arrivée se peint comme celui du départ dont il est le
         ...
         « côté opposé ». */
      const peinture = (cote: string) =>
        i === tentes.length - 1 && tentes.length > 1 && cote === axe && oppose ? oppose : cote;
      for (const [cote, choix] of Object.entries(t.cotes)) {
        ...
        if (nom) poser(dans, nom, cote, peinture(cote), couleursCote[peinture(cote)], voisin);
        ...
        if (basNom) poser(dans, basNom, cote, peinture(cote), couleursCote[peinture(cote)], voisin);
        ...
        if (t.auvents[cote] && VUE.pieceAuvent) poser(dans, VUE.pieceAuvent, cote, peinture(cote), teinteAuvent, voisin);
      }
    });
      ...
      if (fantomes.length > 0) {
        Promise.all(VUE.socle.map((nm) => charger(loader, M, nm))).then((pieces) => {
          ...
          for (const c of fantomes) {
            ...
            for (const p of pieces) fantome.add(p.clone(true));
            murs.add(fantome);
          }
          cadrerRef.current();
        }).catch(() => { if (vivant) setEchec(true); });
