 ...
 * doivent rester.
 */
function retirerFace(scene: THREE.Object3D, azimut: number): void {
  ...
  scene.traverse((o) => {
    ...
    for (let i = 0; i + 2 < nb; i += 3) {
      if (surLaFace(i) && surLaFace(i + 1) && surLaFace(i + 2)) continue;
      for (const j of [i, i + 1, i + 2]) gardes.push(index ? index.getX(j) : j);
    }
    if (gardes.length === nb) return; // rien sur cette face : on ne touche à rien
    ...
    if (gardes.length === 0) { maille.visible = false; return; }
    geo.setIndex(gardes);
  });
}
...
function charger(loader: GLTFLoader, m: ReturnType<typeof trouverModele>, nom: string): Promise<THREE.Group> {
  ...
  if (!p) {
    p = loader.loadAsync(url).then((g) => {
      ...
      if (sans?.piece === nom) {
        for (const cote of sans.cotes) retirerFace(g.scene, angleCote(m, cote));
      }
      return g.scene;
    });
    cache.set(url, p);
...
export default function TenteViewer({ cotes, auvents, bandeaux, couleurs, couleursCote, visuels, visuelsCote, modele, taille, actif, labelChargement, captureRef }: TenteViewerProps) {
  ...
  const piecesAffichees = useRef<{ nom: string; groupe: THREE.Group; cote?: string }[]>([]);
  ...
  useEffect(() => {
    ...
    const poser = (nom: string, cote: string, teinte?: string) => {
    ...
    murs.clear();
    piecesAffichees.current = piecesAffichees.current.filter((p) => (VUE.socle as readonly string[]).includes(p.nom));
