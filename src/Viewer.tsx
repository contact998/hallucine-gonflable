...
function marquerVitre(scene: THREE.Object3D): void {
...
const estVitre = (o: THREE.Object3D) => o.userData.vitre === true;
 ...
 */
function ratioGabarit(geo: THREE.BufferGeometry): number {
  const pos = geo.getAttribute("position");
...
const QUART_DE_TOUR: Record<string, number> = {
  roof: 2, // demi-tour
};
 ...
 */
function centreDuTissu(geo: THREE.BufferGeometry): { x: number; y: number } {
  const uv = geo.getAttribute("uv");
  const index = geo.getIndex();
 ...
 */
function orienter(tex: THREE.Texture, quarts = 0): void {
 ...
 */
function enroulerAutourDeLaTente(groupe: THREE.Object3D, hauteur: number): void {
  const mailles: THREE.Mesh[] = [];
  groupe.traverse((o) => {
    const m = o as THREE.Mesh;
    if (m.isMesh && m.geometry.getAttribute("position")) mailles.push(m);
  });
  if (!mailles.length || hauteur <= 0) return;
