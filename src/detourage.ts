...
const RIEN: Detourage = { detoure: false, fond: null, part: 0 };

function mediane(histo: Uint32Array, total: number): number {
  let cumul = 0;
  for (let v = 0; v < 256; v++) {
    cumul += histo[v];
    if (cumul * 2 >= total) return v;
  }
  return 255;
}
 ...
 */
export function detourerFondUni(
  px: Uint8ClampedArray,
  largeur: number,
  hauteur: number,
  tolerance = TOLERANCE_FOND,
): Detourage {
  const n = largeur * hauteur;
  if (largeur < 3 || hauteur < 3 || px.length < n * 4) return RIEN;

  /* Le pourtour, sans doublon aux quatre coins. */
  const bord: number[] = [];
  for (let x = 0; x < largeur; x++) bord.push(x, (hauteur - 1) * largeur + x);
  for (let y = 1; y < hauteur - 1; y++) bord.push(y * largeur, y * largeur + largeur - 1);

  const hr = new Uint32Array(256), hg = new Uint32Array(256), hb = new Uint32Array(256);
  for (const i of bord) {
    if (px[i * 4 + 3] < 250) return RIEN; // déjà transparent : rien à retirer
    hr[px[i * 4]]++; hg[px[i * 4 + 1]]++; hb[px[i * 4 + 2]]++;
  }
  const fond: [number, number, number] = [
    mediane(hr, bord.length), mediane(hg, bord.length), mediane(hb, bord.length),
  ];

  const t2 = tolerance * tolerance;
  const ecart2 = (i: number): number => {
    const dr = px[i * 4] - fond[0], dg = px[i * 4 + 1] - fond[1], db = px[i * 4 + 2] - fond[2];
    return dr * dr + dg * dg + db * db;
  };

  let proches = 0;
  for (const i of bord) if (ecart2(i) <= t2) proches++;
  if (proches < bord.length * POURTOUR_UNI) return { ...RIEN, fond };
  ...
  let tete = 0, queue = 0;
  for (const i of bord) {
    if (!fondVu[i] && ecart2(i) <= t2) { fondVu[i] = 1; file[queue++] = i; }
  }
  const visiter = (v: number) => {
    if (!fondVu[v] && ecart2(v) <= t2) { fondVu[v] = 1; file[queue++] = v; }
  };
  while (tete < queue) {
    const i = file[tete++];
    const x = i % largeur;
    if (x > 0) visiter(i - 1);
    if (x < largeur - 1) visiter(i + 1);
    if (i >= largeur) visiter(i - largeur);
    if (i + largeur < n) visiter(i + largeur);
  }
  ...
  if (part > 0.985 || part < 0.02) return { ...RIEN, fond, part };

  for (let i = 0; i < n; i++) if (fondVu[i]) px[i * 4 + 3] = 0;
  ...
  for (let i = 0; i < n; i++) {
    ...
    let dLogo = d;
    for (const v of [g, dr, h, b]) if (v >= 0 && !fondVu[v]) dLogo = Math.max(dLogo, Math.sqrt(ecart2(v)));
    if (dLogo <= d) continue; // il est lui-même le plus « logo » : rien à adoucir
...
export function aDeLaTransparence(px: Uint8ClampedArray): boolean {
