 ...
 */
export function echecsAbri<T>(charges: readonly (T | null)[][]): number {
  return charges.flat().filter((c) => c === null).length;
}

async function construireAbri(loader: GLTFLoader, a: Abri): Promise<{ groupe: THREE.Group | null; echecs: number }> {
  try {
 ...
 */
async function habillerAbri(groupe: THREE.Group, pose: VisuelPose, fond: string): Promise<void> {
  try {
    ...
    const parPiece = new Map<string, THREE.CanvasTexture>();
    poserSurAbri(groupe, (maille, nom) => {
      const geo = maille.geometry;
      const donnees = geo.userData as { ratioGabarit?: number; centreTissu?: { x: number; y: number } };
...
function poserSurAbri(
  groupe: THREE.Group,
  texturePour: (maille: THREE.Mesh, nom: string) => THREE.Texture,
  canal: "uv" | "uv1",
): void {
  for (const sous of groupe.children) {
    if (!sous.userData.imprimableAbri) continue;
    const nom = (sous.userData.nomPieceAbri as string | undefined) ?? "";
    sous.traverse((o) => {
      const maille = o as THREE.Mesh;
      if (!maille.isMesh || estVitre(maille)) return;
      if (!maille.geometry.getAttribute(canal)) return;
...
export default function MobilierViewer({ implantation, afficherSol = true, labelChargement, labelEchec = (n: number) => `${n} élément(s) de la scène n’ont pas pu être chargés.`, captureRef, abri, coteActif, ecran, arche, habillages, visuels, libellesOutils, nomFichierImage, effacerParois }: Props) {
  ...
  useEffect(() => {
    ...
    const cadrer = (azimut?: number) => {
      ...
      if (azimut === undefined && vueFixee.current) { appliquerVue(vueFixee.current, false); return; }
  ...
  useEffect(() => {
    ...
    const coureursPromises = Promise.all(
      (arche ? coureursArche() : []).map((pose) =>
        chargerGLB(o.loader, urlPersonne(pose.modele.fichier))
  ...
  useEffect(() => {
    if (!pret || !abri || !coteActif) return;
    const m = modeleTente(abri.modele);
    if (!(m.cotes as readonly string[]).includes(coteActif)) return;
