...
export function estMembrane(profondeurs, normaleY) {
...
export function groupeDeProfondeur(s) {
    if (s < -SEUIL_FACE)
        return GROUPE_AVANT;
    if (s > SEUIL_FACE)
        return GROUPE_ARRIERE;
    return GROUPE_FLANC;
}
/**
 ...
 */
export function preparerFacesArche(groupe, m) {
    ...
    groupe.traverse((o) => {
        ...
        if (deja.has(maille.geometry))
            maille.geometry = maille.geometry.clone();
        ...
        for (let t = 0; t + 2 < index.length; t += 3) {
            ...
            if (longueur > 0 && estMembrane([profondeur[a], profondeur[b], profondeur[c]], normale.y / longueur))
                continue;
        ...
        parGroupe.forEach((liste, g) => {
            if (liste.length)
                geo.addGroup(debut, liste.length, g);
            debut += liste.length;
        });
    });
}
 ...
 * sur un shader cassé.
 */
export function matiereArche() {
    const m = new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.82, metalness: 0, side: THREE.DoubleSide });
    m.onBeforeCompile = (shader) => {
        shader.fragmentShader = shader.fragmentShader.replace("#include <normal_fragment_begin>", THREE.ShaderChunk.normal_fragment_begin.replace("vec3 normal = normalize( vNormal );", "vec3 normal = normalize( vNormal );\n\tif ( dot( normal, vViewPosition ) * faceDirection < 0.0 ) normal = - normal;"));
    };
