...
export const RATIOS_PROJECTION = [
 ...
 */
export function bandeauMasque(twMm, ratio) {
    if (ratio <= RATIO_REF + 1e-9)
        return 0;
 ...
 */
export function boudinEtanche(twMm) {
    if (twMm <= 3000)
        return 220;
    if (twMm <= 4500)
        return 370;
 ...
 */
export function boudinSoufflerie(twMm) {
    if (twMm <= 11000)
        return 1590;
    if (twMm <= 13000)
        return 1910;
...
export const boudinPourLargeur = (gamme, twMm) => gamme === "etanche" ? boudinEtanche(twMm) : boudinSoufflerie(twMm);
/** Plancher du bas d'image : le demi-boudin, arrondi au cran supérieur. */
export function planchierBasImage(gamme, boudinMm) {
