...
export const VUES = ["face", "cote", "dessus", "troisQuarts"];
/**
...
export function directionVue(vue, azimutFace) {
    const { decalage, elevation } = REGLAGE[vue];
    const az = azimutFace + decalage;
    return new THREE.Vector3(Math.cos(az) * Math.cos(elevation), Math.sin(az) * Math.cos(elevation), Math.sin(elevation));
}
/**
 ...
 */
export function poseVue(vue, boite, { azimutFace, fov, aspect }) {
 ...
 * côté (`viseeCote`).
 */
export function interpolerPose(de, vers, k) {
    ...
    const position = cible.clone().add(new THREE.Vector3(Math.cos(az) * Math.cos(el), Math.sin(az) * Math.cos(el), Math.sin(el)).multiplyScalar(r));
    return { position, cible };
}
/** Accélère puis freine : un départ sec et une arrivée brusque se lisent comme
 *  un saut raté. */
const adoucir = (t) => (t < 0.5 ? 2 * t * t : 1 - (-2 * t + 2) ** 2 / 2);
/**
 ...
 */
export function dureeTransition() {
...
export class TransitionVue {
    ...
    lancer(de, vers, maintenant, duree) {
